...
fnames=[os.path.join(training_cats_direc,fname) for fname in os.listdir(train_cats_direc)]

#Choose one image to "augment"
img_paths = fnames[3]

#Read the image and resize it
img1 = image.load_img(img_paths,target_size=(150,150))

#Convert to a numpy array of desired shape
...
for batch in datagen.flow(x,batch_size=1):
    plt.figure(i)
    imgplot = plt.imshow(img1.array_to_img(batch[0]))
    i+=1
    if i%4==0:
        break
plt.show()
#A dropout can also be added to this neural net and can be implemented.
